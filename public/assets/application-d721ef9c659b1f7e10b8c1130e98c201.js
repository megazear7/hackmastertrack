    ...
    return $(".specializations").each(function(index, specialization) {
      ...
      if (character_id !== "" && stat_name !== "") {
        return $.getJSON("/specialization/retrieve" + ("?character_id=" + character_id) + ("&item_id=" + item_id) + ("&stat_name=" + stat_name), function(data) {
          ...
          for (i = 0, ref = data.value; 0 <= ref ? i < ref : i > ref; 0 <= ref ? i++ : i--) {
            $(specialization).find(".current-value").append("<span class='glyphicon glyphicon-stop'></span>");
          }
          results = [];
          for (j = ref1 = data.value; ref1 <= 5 ? j < 5 : j > 5; ref1 <= 5 ? j++ : j--) {
            results.push($(specialization).find(".current-value").append("<span class='glyphicon glyphicon-unchecked'></span>"));
          }
          return results;
        });
      }
